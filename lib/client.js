...
`
  ...
  function playVoice(url) {
    ...
    try {
      ...
      audio.addEventListener('error', noteVoiceFinished, { once: true })
      // Safety net: a stalled or truncated stream may never fire `ended`, and the
      // box must not be held open forever waiting for it.
      ...
      if (started && typeof started.catch === 'function') started.catch(() => noteVoiceFinished())
    } catch {
      // Autoplay restrictions or a missing codec must never break the widget.
      noteVoiceFinished()
    }
  }
  ...
  function playClick() {
    try {
      if (!clickAudio) clickAudio = new Audio(`${UI}/click.wav`)
   ...
   *
   * The logo occupies the artwork's y 72.8%–90.9%; the inner box spans 7%–96%, so
   * (72.8-7)/89 = 0.739 is the last safe fraction. 0.72 leaves a little margin.
   */
  const LOGO_SAFE_SHARE = 0.72
  ...
  function fitFooter(dlgW, cap, full) {
    ...
    // its top edge sits ~73% of the way down the inner box.
    const budget = full ? Math.max(6, Math.round(cap * LOGO_SAFE_SHARE)) : cap
  ...
  function revertToDefaultSprite() {
    ...
    setSprite({
      file,
      url: `${ROUTE}/asset/sprite?pack=${encodeURIComponent(pack.id)}&file=${encodeURIComponent(file)}`,
