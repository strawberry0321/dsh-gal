...
* 两个文件夹也可以叫 `立绘\` / `语音\`，或 `立ち絵\` / `ボイス\` —— 中文、日文目录名都认，
  不用为了插件改名（同名的英文目录优先）。台词表放包目录或语音目录里都能找到。
...
有的立绘是朝左画的，挂在右下角时想让它朝右，就整套左右镜像一遍。仓库里带了这个工具
（无损：解码 → 逐行翻转 → 重新编码，写完再逐像素回验，而且镜像两次就等于还原）：

```powershell
node scripts/mirror-sprites.mjs "D:\path\to\pack\立绘"          # 原地镜像
node scripts/mirror-sprites.mjs "D:\path\to\pack\立绘" --out "D:\path\to\pack\立绘-镜像"
```

只处理 8 位、非隔行的 PNG（灰度 / RGB / RGBA）；调色板、16 位、隔行图会直接报错而不是悄悄改坏。
镜像后文件名不变，所以 `pack.json` 里的 `defaultSprite` 不用改。

### 语音顺序与权重

默认**洗牌池**：把整个语音包洗成一轮，逐条不放回地播；一轮放完自动重洗，新一轮不会拿上一轮
结尾那句开头。所以同一句台词在整轮放完之前不会再次出现——400 次点击里任何一句最多 1 次。
想改回"每次都独立随机"，设定面板里把「语音顺序」切成 `纯随机` 即可（立刻生效）。

要让某几句**多放 / 少放 / 不播**，在包目录里放 `weights.json`：
...
| 写法 | 含义 |
|---|---|
| 键 = 语音文件名（带不带 `.wav` 都行，如 `mas0033`） | 只调这一条 |
| 键 = 台词原文（**完全匹配**，如 `……。`） | 调说这句话的所有语音 |
| `1`（默认） | 正常频率 |
| `0.25` | 四分之一频率 |
| `3` | 三倍频率（一轮里出现 3 次，位置随机且不会连在一起） |
| `0` | 这一条永不播放 |

* 只写要调的那几条就行，其余默认 1。
* **不做任何自动合并**：`……。` 和 `……` 是两条不同的键；标点、空格、全半角都不归一化。
* 文件缺失 / JSON 语法错误 / 键写错 → 那一项按 1 处理，不会报错、不会让某句消失。
* 设定面板的「语音包」下面会显示 `· 权重 n 条`，方便确认文件生效了。
...
替换包里的**同名**文件（例如把整套立绘换成镜像版）之后，**刷新页面**就能看到新图：立绘与语音的
URL 带文件版本号（修改时间 + 大小），换了内容就是换了地址，不会读到浏览器缓存里的旧素材。
...
| `volume` | 0–1 的小数 |
| `lang` | `ja`（日文）或 `zh`（中文） |
| `voiceOrder` | 语音抽取方式：`shuffle`（洗牌池，默认）或 `random`（纯随机） |
